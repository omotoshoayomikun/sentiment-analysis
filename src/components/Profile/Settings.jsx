...
function Settings({loading, value, handleOnchange, handleSubmit}) {
    ...
    <>
      <div className="h-[340px] overflow-hidden overflow-y-auto">
        <div className="grid grid-cols-2 gap-5">
            {
                Inputs.map((input) => (
                    <Input key={input.id} {...input} value={value[input.name]} handleOnchange={(e) => handleOnchange(e, input.name)} />
                ))
            }
        </div>
        <Btn title="Update" handleClick={handleSubmit}  disabled={loading} styles={{width: "200px", background: "#007bff"}} />
      </div>
    </>
  );
}
